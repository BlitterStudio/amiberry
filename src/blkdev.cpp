...
{
	if (currprefs.sound_volume_cd == changed_prefs.sound_volume_cd)
		return;
...
int sys_command_cd_read(int unitnum, uae_u8* data, int block, int size)
{
	...
	{
		uae_u8 cmd1[12] = {0x28, 0, static_cast<uae_u8>(block >> 24, block >> 16, block >> 8, block >> 0, 0, size >> 8, size >> 0, 0, 0, 0)};
...
int sys_command_cd_rawread(int unitnum, uae_u8* data, int block, int size, int sectorsize)
{
	...
	{
		uae_u8 cmd[12] = {0xbe, 0, static_cast<uae_u8>(block >> 24, block >> 16, block >> 8, block >> 0, size >> 16, size >> 8, size >> 0, 0x10, 0, 0)};
...
int sys_command_read(int unitnum, uae_u8* data, int block, int size)
{
	...
	if (device_func[unitnum]->read == NULL)
	{
		uae_u8 cmd[12] = {0xa8, 0, 0, 0, 0, 0, static_cast<uae_u8>(size >> 24, size >> 16, size >> 8, size >> 0, 0, 0)};
