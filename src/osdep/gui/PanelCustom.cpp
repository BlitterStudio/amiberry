...
public:
	void action(const gcn::ActionEvent& actionEvent) override
	{
		...
		{
		case 0:
                 tempmap = changed_prefs.jports[SelectedPort].amiberry_custom_none; break;
		case 1:
                 tempmap = changed_prefs.jports[SelectedPort].amiberry_custom_hotkey; break;
		case 2:
                 tempmap = changed_prefs.jports[SelectedPort].amiberry_custom_left_trigger; break;
		case 3:
                 tempmap = changed_prefs.jports[SelectedPort].amiberry_custom_right_trigger; break;
                }
       ...
       //     
		...
		{
		case 0:
                 changed_prefs.jports[SelectedPort].amiberry_custom_none = tempmap;  break;
		case 1:
                 changed_prefs.jports[SelectedPort].amiberry_custom_hotkey = tempmap;  break;
		case 2:
                 changed_prefs.jports[SelectedPort].amiberry_custom_left_trigger = tempmap; break;
		case 3:
                 changed_prefs.jports[SelectedPort].amiberry_custom_right_trigger = tempmap; break;
                }
      ...
      // update the joystick port  , and disable those which are not available
        ...
        {       hostjoyid = changed_prefs.jports[SelectedPort].id - JSEM_JOYS - 1;
                ...
                {   
                    ...
                    switch (n)
                    {
                    case  0:  { temp_button = host_input_buttons[hostjoyid].dpad_up;        break;}
                    case  1:  { temp_button = host_input_buttons[hostjoyid].dpad_down;      break;}
                    case  2:  { temp_button = host_input_buttons[hostjoyid].dpad_left;      break;}
                    case  3:  { temp_button = host_input_buttons[hostjoyid].dpad_right;     break;}
                    case  4:  { temp_button = host_input_buttons[hostjoyid].select_button;  break;}
                    case  5:  { temp_button = host_input_buttons[hostjoyid].left_shoulder;  break;}
                    case  6:  { temp_button = host_input_buttons[hostjoyid].lstick_button;  break;}
                    case  7:  { temp_button = host_input_buttons[hostjoyid].north_button;   break;}
                    case  8:  { temp_button = host_input_buttons[hostjoyid].south_button;   break;}
                    case  9:  { temp_button = host_input_buttons[hostjoyid].east_button;    break;}
                    case 10:  { temp_button = host_input_buttons[hostjoyid].west_button;    break;}
                    case 11:  { temp_button = host_input_buttons[hostjoyid].start_button;   break;}
                    case 12:  { temp_button = host_input_buttons[hostjoyid].right_shoulder; break;}
                    case 13:  { temp_button = host_input_buttons[hostjoyid].rstick_button;  break;}
                    }          
                    ...
                    { cboCustomAction[n]->setListModel(&CustomEventList_HotKey);  
                      ...
                      cboCustomAction[n]->setSelected(0);}     
                         ...
                         else if (temp_button == host_input_buttons[hostjoyid].quit_button && temp_button != -1 && SelectedFunction == 1 && changed_prefs.amiberry_use_retroarch_quit == true)
                    { cboCustomAction[n]->setListModel(&CustomEventList_Quit);
                      cboCustomAction[n]->setEnabled(false);     
                      cboCustomAction[n]->setSelected(0);}   
                
                         else if (temp_button == host_input_buttons[hostjoyid].menu_button && temp_button != -1 && SelectedFunction == 1  && changed_prefs.amiberry_use_retroarch_menu == true)
                    { cboCustomAction[n]->setListModel(&CustomEventList_Menu);
                      cboCustomAction[n]->setEnabled(false);  
                      cboCustomAction[n]->setSelected(0);}     
                    
                    else if (temp_button == host_input_buttons[hostjoyid].reset_button && temp_button != -1 && SelectedFunction == 1  && changed_prefs.amiberry_use_retroarch_reset == true)
        ...
        {    switch (z)
            {
                case 0:  { eventnum = tempmap.dpad_up_action; break; }
                case 1:  { eventnum = tempmap.dpad_down_action; break; }
                case 2:  { eventnum = tempmap.dpad_left_action; break; }
                case 3:  { eventnum = tempmap.dpad_right_action; break; }
                case 4:  { eventnum = tempmap.select_action;  break; }
                case 5:  { eventnum = tempmap.left_shoulder_action; break; }
                case 6:  { eventnum = tempmap.lstick_select_action; break; }
                case 7:  { eventnum = tempmap.north_action; break; }
                case 8:  { eventnum = tempmap.south_action; break; }
                case 9:  { eventnum = tempmap.east_action; break; }
                case 10: { eventnum = tempmap.west_action; break; }
                case 11: { eventnum = tempmap.start_action; break; }
                case 12: { eventnum = tempmap.right_shoulder_action; break; }
                case 13: { eventnum = tempmap.rstick_select_action; break; }
            }
            x = find_in_array(RemapEventList, RemapEventListSize, eventnum);            
...
{
	helptext.clear();
	helptext.push_back("Set up custom input actions for each Amiga port, such as Keyboard remapping, or emulator functions.");
	helptext.push_back("");
	...
	helptext.push_back("");
	helptext.push_back("Buttons which are not available on this device (detected with RetroArch configuration files) are unavailable to remap.");
	helptext.push_back("");
