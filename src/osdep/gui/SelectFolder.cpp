...
#ifdef USE_OPENGL
			touch_event.button.x = float(gui_graphics->getTargetPlaneWidth()) * int(event.tfinger.x);
			touch_event.button.y = float(gui_graphics->getTargetPlaneHeight()) * int(event.tfinger.y);
#else
			touch_event.button.x = gui_graphics->getTarget()->w * int(event.tfinger.x);
			touch_event.button.y = gui_graphics->getTarget()->h * int(event.tfinger.y);
#endif
