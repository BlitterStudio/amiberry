    ...
    while (std::getline(readFile, line))
    {
        ...
        if (option != line) // exit if we got no result from splitting the string
        {
           ...
           //  time to get the output number
            ...
            {   tempbutton = abs(atol(param.c_str()));         } // gets the parameter
            ...
            if (option == find_setting)             
               { break; }   
            else if (find_setting != "count_hats")
               {tempbutton = -1;}
			...
			if (zfile_exists(ControlConfig))
                        {
                            ...
                            host_input_buttons[cpt].hotkey_button    = find_retroarch("input_enable_hotkey_btn" ,ControlConfig, host_input_buttons[cpt]);

                            host_input_buttons[cpt].quit_button      = find_retroarch("input_exit_emulator_btn" ,ControlConfig, host_input_buttons[cpt]);
                            host_input_buttons[cpt].menu_button      = find_retroarch("input_menu_toggle_btn"   ,ControlConfig, host_input_buttons[cpt]);
                            ...
                            host_input_buttons[cpt].lstick_axis_x    = find_retroarch("input_l_x_plus_axis"     ,ControlConfig, host_input_buttons[cpt]);       
                        ...
                        {   host_input_buttons[cpt].lstick_axis_x    = find_retroarch("input_right_axis"        ,ControlConfig, host_input_buttons[cpt]);  }                            
                            ...
                            host_input_buttons[cpt].lstick_axis_y    = find_retroarch("input_l_y_plus_axis"     ,ControlConfig, host_input_buttons[cpt]);       
                        ...
                        {   host_input_buttons[cpt].lstick_axis_y    = find_retroarch("input_down_axis"         ,ControlConfig, host_input_buttons[cpt]);  }                            
                            ...
                            host_input_buttons[cpt].rstick_axis_x    = find_retroarch("input_r_x_plus_axis"     ,ControlConfig, host_input_buttons[cpt]);       
                            host_input_buttons[cpt].rstick_axis_y    = find_retroarch("input_r_y_plus_axis"     ,ControlConfig, host_input_buttons[cpt]);       
...
static void read_joystick(void)
{
	for (int joyid = 0; joyid < MAX_JPORTS; joyid++)
	{
		...
		else if (jsem_isjoy(joyid, &currprefs) != -1)
		{
			...
			int hostjoyid = currprefs.jports[joyid].id - JSEM_JOYS - numKeysAsJoys;
                        
                        static struct host_input_button current_controller_map;
                        
                        current_controller_map = host_input_buttons[hostjoyid];
                             
                        if (current_controller_map.east_button    == current_controller_map.hotkey_button ) {current_controller_map.east_button = -1;}
                        if (current_controller_map.south_button   == current_controller_map.hotkey_button ) {current_controller_map.south_button = -1;}
                        if (current_controller_map.north_button   == current_controller_map.hotkey_button ) {current_controller_map.north_button = -1;}
                        if (current_controller_map.west_button    == current_controller_map.hotkey_button ) {current_controller_map.west_button = -1;}
                        if (current_controller_map.dpad_left      == current_controller_map.hotkey_button ) {current_controller_map.dpad_left = -1;}
                        if (current_controller_map.dpad_right     == current_controller_map.hotkey_button ) {current_controller_map.dpad_right = -1;}
                        if (current_controller_map.dpad_up        == current_controller_map.hotkey_button ) {current_controller_map.dpad_up = -1;}
                        if (current_controller_map.dpad_down      == current_controller_map.hotkey_button ) {current_controller_map.dpad_down = -1;}
                        if (current_controller_map.select_button  == current_controller_map.hotkey_button ) {current_controller_map.select_button = -1;}
                        if (current_controller_map.start_button   == current_controller_map.hotkey_button ) {current_controller_map.start_button = -1;}
                        if (current_controller_map.left_trigger   == current_controller_map.hotkey_button ) {current_controller_map.left_trigger = -1;}
                        if (current_controller_map.right_trigger  == current_controller_map.hotkey_button ) {current_controller_map.right_trigger = -1;}
                        if (current_controller_map.lstick_button  == current_controller_map.hotkey_button ) {current_controller_map.lstick_button = -1;}
                        if (current_controller_map.rstick_button  == current_controller_map.hotkey_button ) {current_controller_map.rstick_button = -1;}
                                
                        if (current_controller_map.left_shoulder  == current_controller_map.hotkey_button ) {current_controller_map.left_shoulder = -1;}
                        if (current_controller_map.right_shoulder == current_controller_map.hotkey_button ) {current_controller_map.right_shoulder = -1;}
			...
			int held_offset = 0;
                        ...
                        {held_offset = REMAP_BUTTONS;
                         setjoybuttonstate(hostjoyid + 1, 14 ,(SDL_JoystickGetButton(Joysticktable[hostjoyid], current_controller_map.menu_button) & 1)); // menu button
                         setjoybuttonstate(hostjoyid + 1, 15 ,(SDL_JoystickGetButton(Joysticktable[hostjoyid], current_controller_map.quit_button) & 1)); // quit button
                         setjoybuttonstate(hostjoyid + 1, 30 ,(SDL_JoystickGetButton(Joysticktable[hostjoyid], current_controller_map.reset_button) & 1)); // reest button
                        }                      
			...
			setjoybuttonstate(hostjoyid + 1, 4 + held_offset,
			                  (SDL_JoystickGetButton(Joysticktable[hostjoyid], current_controller_map.left_shoulder) & 1)); // left shoulder
			setjoybuttonstate(hostjoyid + 1, 5 + held_offset,
			                  (SDL_JoystickGetButton(Joysticktable[hostjoyid], current_controller_map.right_shoulder) & 1)); // right shoulder
                               ...
                               bool joymouseswap)
{
	...
	// directions
        ...
        {   
            ...
            {
                    thismap[0].left_shoulder_action = (thismap[0].left_shoulder_action
                                                               ? thismap[0].left_shoulder_action   : INPUTEVENT_KEY_SPACE);
                    thismap[0].right_shoulder_action = (thismap[0].right_shoulder_action
...
//	thismap[3] = currprefs.jports[port].amiberry_custom_right_trigger; // grab the 'right trigger' options for the current map
	...
	for (int n = 0; n < 2; ++n) /// temporarily limited to '2' only
	{
     ...
     // if using retroarch options
        if (currprefs.amiberry_use_retroarch_menu  == true) {setid(uid, num, ID_BUTTON_OFFSET + 14, 0, port, INPUTEVENT_SPC_ENTERGUI, gp);}
        if (currprefs.amiberry_use_retroarch_quit  == true) {setid(uid, num, ID_BUTTON_OFFSET + 15, 0, port, INPUTEVENT_SPC_QUIT, gp);    }
        if (currprefs.amiberry_use_retroarch_reset == true) {setid(uid, num, ID_BUTTON_OFFSET + 30, 0, port, INPUTEVENT_SPC_SOFTRESET, gp);    }
