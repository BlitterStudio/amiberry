...
void render_panel_rtg()
{
    ...
    if (ImGui::Combo("Board", &current_board_idx, rtg_boards_c.data(), (int)rtg_boards_c.size())) {
    ...
    bool vram_disabled = (changed_prefs.address_space_24 && changed_prefs.rtgboards[0].rtgmem_type != 0);
    ImGui::BeginDisabled(vram_disabled || current_board_idx == 0);
    
    // Display textual representation of the slider value
    char vram_label[32];
    sprintf(vram_label, "%d MB", 1 << slider_val);
    
    if (ImGui::SliderInt("VRAM size", &slider_val, 0, max_slider_val, vram_label)) {
    ...
    // 16-bit Mode
    const char* rtg_16bit_modes[] = { "15/16-bit", "All", "R5G6B5PC (*)", "R5G5B5PC", "R5G6B5", "R5G5B5", "B5G6R5PC", "B5G5R5PC" };
    int idx16 = 0;
    ...
    if (ImGui::Combo("##16bit", &idx16, rtg_16bit_modes, IM_ARRAYSIZE(rtg_16bit_modes))) {
        ...
        if (idx16 == 1) mask |= RGBFF_R5G6B5PC | RGBFF_R5G5B5PC | RGBFF_R5G6B5 | RGBFF_R5G5B5 | RGBFF_B5G6R5PC | RGBFF_B5G5R5PC;
    ...
    if (ImGui::Combo("Refresh rate", &current_rate_idx, rtg_refreshrates, IM_ARRAYSIZE(rtg_refreshrates))) {
        switch (current_rate_idx) {
            case 0: changed_prefs.rtgvblankrate = 0; break;
            case 1: changed_prefs.rtgvblankrate = -1; break;
            case 2: changed_prefs.rtgvblankrate = 50; break;
            case 3: changed_prefs.rtgvblankrate = 60; break;
            case 4: changed_prefs.rtgvblankrate = 70; break;
