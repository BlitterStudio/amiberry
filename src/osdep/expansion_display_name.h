...
// on the NULL dereference. Every site that renders, compares or logs a board
// name goes through this helper instead of reading friendlyname raw: it
// returns the friendly name when it is a usable string, the manufacturer
// when only that exists, and a generic placeholder otherwise, so the
// returned pointer is never NULL and never empty.
//
...
// files and the standalone tests can all include it. TCHAR is char on
// every Amiberry platform, so the narrow signature covers every build.
