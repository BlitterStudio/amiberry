...
static void cfgfile_writeramboard(struct uae_prefs *prefs, struct zfile *f, const TCHAR *name, int num, struct ramboard *rb)
{
	...
	if (rb->no_reset_unmap) {
		if (tmp2[0])
			_tcscat(tmp2, _T(","));
		_tcscat(tmp2, _T("no_reset_unmap=true"));
	}
	if (rb->nodma) {
		if (tmp2[0])
			_tcscat(tmp2, _T(","));
		_tcscat(tmp2, _T("nodma=true"));
	}
	if (rb->force16bit) {
		if (tmp2[0])
			_tcscat(tmp2, _T(","));
		_tcscat(tmp2, _T("force16bit=true"));
	}
	if (rb->fault) {
		if (tmp2[0])
			_tcscat(tmp2, _T(","));
	...
	if (!_tcsicmp(tmp1, _T("chipmem_options")) || !_tcsicmp(tmp1, _T("bogomem_options"))) {
		if (!rb->chipramtiming) {
			if (tmp2[0])
				_tcscat(tmp2, _T(","));
			_tcscat(tmp2, _T("slow=false"));
		}
	} else {
		if (rb->chipramtiming) {
			if (tmp2[0])
				_tcscat(tmp2, _T(","));
			_tcscat(tmp2, _T("slow=true"));
		}
	}
