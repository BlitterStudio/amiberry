...
int get_filesys_unitconfig (struct uae_prefs *p, int index, struct mountedinfo *mi)
{
  ...
  if (!ui) {
      ...
      ui->hf.fd = fopen (uci->rootdir, _T("r+b"));
	    if (ui->hf.fd == 0) {
		    mi->ismedia = false;
		    mi->ismounted = true;
      ...
      ui->hf.virtsize = ftell(ui->hf.fd);
      ui->hf.virtsize &= ~(ui->hf.blocksize - 1);
...
static int exalldo (uaecptr exalldata, uae_u32 exalldatasize, uae_u32 type, uaecptr control, Unit *unit, a_inode *aino)
{
  ...
  if (exalldata + exalldatasize - exp < size + size2)
	...
	write_log (_T("ID=%d, %d, %08x: '%s'%s\n"),
		get_long (control + 4), get_long (control + 0), exp, x, aino->dir ? _T(" [DIR]") : _T(""));
#endif
  ...
  action_rename_disk (Unit *unit, dpacket packet)
{
  ...
  /* get volume name */
  xfree (unit->ui.volname);
  unit->ui.volname = my_strdup(bstr1 (name));
...
uae_u8 *restore_filesys (uae_u8 *src)
{
  int type, devno;
  UnitInfo *ui;
  TCHAR *devname = 0, *volname = 0, *rootdir = 0;
  int bootpri;
  bool readonly;
	uae_u32 startup;
