...
void blitdofast_fe (uaecptr pta, uaecptr ptb, uaecptr ptc, uaecptr ptd, struct bltinfo *_GCCRES_ b)
{
  register unsigned int i,j;
  uae_u32 totald = 0;
  uae_u32 srcb = b->bltbhold;
  ...
  for (j = b->vblitsize; j--;) 
  {
	  for (i = b->hblitsize; i--;) 
	  {
		  ...
		  if (ptc) { 
		    srcc = CHIPMEM_AGNUS_WGET_CUSTOM (ptc); 
		    ptc += 2; 
		  }
		  if (ptb) {
			  uae_u32 bltbdat; 
			  b->bltbdat = bltbdat = CHIPMEM_AGNUS_WGET_CUSTOM (ptb); 
			  ptb += 2;
			  srcb = (((uae_u32)prevb << 16) | bltbdat) >> b->blitbshift;
			  prevb = bltbdat;
		  }
		  if (pta) { 
		    b->bltadat = bltadat = CHIPMEM_AGNUS_WGET_CUSTOM (pta); 
		    pta += 2; 
		  } else { 
		    bltadat = b->bltadat; 
		  }
