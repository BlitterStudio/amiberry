...
static void createrootblock(uae_u8* sector, const TCHAR* disk_name)
{
	...
	strcpy(reinterpret_cast<char*>(sector) + 433, dn2);
	sector[508 + 3] = 1;
...
static int createdirheaderblock(uae_u8* sector, int parent, const char* filename, uae_u8* bitmap, int* prevblock)
{
	...
	strcpy(reinterpret_cast<char *>(sector) + 512 - 79, filename);
...
static int createfileheaderblock(struct zfile* z, uae_u8* sector, int parent, const char* filename, struct zfile* src, uae_u8* bitmap, int* prevblock)
{
	...
	strcpy(reinterpret_cast<char *>(sector) + FS_FLOPPY_BLOCKSIZE - 79, filename);
...
static int read_header_ext2(struct zfile* diskfile, trackid* trackdata, int* num_tracks, int* ddhd)
{
	...
	if (strncmp(reinterpret_cast<char *>(buffer), "UAE-1ADF", 8))
		return 0;
...
static int iswritefileempty(struct uae_prefs* p, const TCHAR* name)
{
	...
	if (strncmp(static_cast<uae_char*>(buffer), "UAE-1ADF", 8))
		return 0;
...
static bool diskfile_iswriteprotect(struct uae_prefs* p, const TCHAR* fname, int* needwritefile, drive_type* drvtype)
{
	...
	*drvtype = DRV_35_DD;
	...
	zfile_fclose(zf1);
	if (strncmp(static_cast<uae_char*>(buffer), "CAPS", 4) == 0) {
		*needwritefile = 1;
		return wrprot2;
	}
	if (strncmp(static_cast<uae_char*>(buffer), "SCP", 3) == 0) {
		*needwritefile = 1;
		return wrprot2;
	}
	if (strncmp(static_cast<uae_char *>(buffer), "Formatted Disk Image file", 25) == 0)
	{
		*needwritefile = 1;
		return wrprot2;
	}
	if (strncmp(static_cast<uae_char *>(buffer), "UAE-1ADF", 8) == 0)
	{
		if (wrprot1)
			return wrprot2;
		return wrprot1;
	}
	if (strncmp(static_cast<uae_char *>(buffer), "UAE--ADF", 8) == 0)
	{
		*needwritefile = 1;
		return wrprot2;
	}
...
static int drive_insert(drive* drv, struct uae_prefs* p, int dnum, const TCHAR* fname, bool fake, bool forcedwriteprotect)
{
	...
	else if (strncmp(reinterpret_cast<char*>(buffer), "UAE-1ADF", 8) == 0)
	{
	...
	else if (strncmp(reinterpret_cast<char *>(buffer), "UAE--ADF", 8) == 0)
	{
		int offs = 160 * 4 + 8;
		int i;
...
static void drive_fill_bigbuf(drive* drv, int force)
{
	...
	if (drv->writediskfile && drv->writetrackdata[tr].bitlen > 0)
	{
		...
		read_floppy_data(drv->writediskfile, drv->filetype, wti, 0, reinterpret_cast<uae_u8*>(drv->bigmfmbuf), (wti->bitlen + 7) / 8);
		for (i = 0; i < (drv->tracklen + 15) / 16; i++)
		{
			uae_u16* mfm = drv->bigmfmbuf + i;
			uae_u8* data = reinterpret_cast<uae_u8 *>(mfm);
			*mfm = 256 * *data + *(data + 1);
		}
	}
	...
	else
	{
		...
		read_floppy_data(drv->diskfile, drv->filetype, ti, 0, reinterpret_cast<uae_u8*>(drv->bigmfmbuf + base_offset), (ti->bitlen + 7) / 8);
...
static void diskfile_update(struct zfile* diskfile, trackid* ti, int len, image_tracktype type)
{
	...
	buf[2] = 0;
	buf[3] = ti->type;
	do_put_mem_long(reinterpret_cast<uae_u32 *>(buf + 4), ti->len);
	do_put_mem_long(reinterpret_cast<uae_u32 *>(buf + 8), ti->bitlen);
...
{
	strcpy(reinterpret_cast<char*>(dst), "DOS");
	dst[3] = ffs ? 1 : 0;
	if (bootable)
...
static void floppy_get_rootblock(uae_u8* dst, int block, const TCHAR* disk_name, drive_type adftype)
{
	...
	dst[432] = strlen(s);
	strcpy(reinterpret_cast<char*>(dst) + 433, s);
	xfree (s);
	dst[508 + 3] = 1;
	disk_date(dst + 420);
