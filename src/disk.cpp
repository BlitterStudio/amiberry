...
static void floppy_get_rootblock (uae_u8 *dst, int block, const TCHAR *disk_name, drive_type adftype)
{
  ...
  if(disk_name && _tcslen (disk_name) > 0) {
	  dst[432] = strlen (disk_name);
	  strcpy ((char*)dst + 433, disk_name);
  } else {
	  dst[432] = strlen ("empty");
	  strcpy ((char*)dst + 433, "empty");
  }
	dst[508 + 3] = 1;
	disk_date (dst + 420);
...
void disk_creatediskfile (const TCHAR *name, int type, drive_type adftype, const TCHAR *disk_name, bool ffs, bool bootable)
{
  ...
  if (f && chunk) {
      ...
      l = track_len;
			zfile_fwrite ((char *)"UAE-1ADF", 8, 1, f);
...
STATIC_INLINE void fetch_DISK_select(uae_u8 data)
{
	selected = (data >> 3) & 15;
	side = 1 - ((data >> 2) & 1);
	direction = (data >> 1) & 1;
    ...
    if (dmaen (DMA_DISK) && dskdmaen == DSKDMA_WRITE && dsklength > 0 && fifo_filled) {
      ...
      if (!bitoffset) {
				...
				if (currprefs.floppy_speed > 100 && !fifo_inuse[0] && !fifo_inuse[1] && !fifo_inuse[2]) {
					while (!fifo_inuse[2]) {
						uae_u16 w = CHIPMEM_AGNUS_WGET_CUSTOM (dskpt);
						DSKDAT (w);
						dskpt += 2;
					}
				}
	...
	if (dmaen (DMA_DISK) && bitoffset == 15 && dma_enable && dskdmaen == DSKDMA_READ && dsklength >= 0) {
		if (dsklength > 0) {
			...
			if (currprefs.floppy_speed > 100 && fifo_inuse[0] && fifo_inuse[1] && fifo_inuse[2]) {
				while (fifo_inuse[0]) {
					uae_u16 w = DSKDATR ();
					CHIPMEM_AGNUS_WPUT_CUSTOM (dskpt, w);
					dskpt += 2;
				}
			}
			if (disk_fifostatus () > 0) {
    ...
    for (dr = 0; dr < MAX_FLOPPY_DRIVES; dr++) {
      ...
      pos = drv->mfmpos & ~15;
	    drive_fill_bigbuf (drv, 0);
	      ...
	      while (dsklength-- > 0) {
	        CHIPMEM_AGNUS_WPUT_CUSTOM (dskpt, drv->bigmfmbuf[pos >> 4]);
	        dskpt += 2;
	        pos += 16;
	        pos %= drv->tracklen;
	      }
          ...
          uae_u16 w = CHIPMEM_AGNUS_WGET_CUSTOM (dskpt + i * 2);
	  ...
	  if (!done && noselected) {
	    while (dsklength-- > 0) {
	        ...
	        CHIPMEM_AGNUS_WPUT_CUSTOM (dskpt, 0);
		    }
	      dskpt += 2;
	    }
	    INTREQ (0x8000 | 0x1000);
...
uae_u8 *restore_disk(int num,uae_u8 *src)
{
  ...
  if (state & 2) {
  	...
  	changed_prefs.floppyslots[num].dfxtype = (drive_type) -1;
  } else {
...
uae_u8 *save_disk2 (int num, int *len, uae_u8 *dstptr)
{
	...
	if (dstptr)
		...
		dstbak = dst = xmalloc (uae_u8, 4 + 2 + 4 + 2 + 4 + 4 + size);
