...
static void returncycles_exception (const char *s, int cycles)
{
    if (using_ce || using_ce020)
    {
#if 0
...
static const char *gen_nextiword (int flags)
{
    ...
    else
    {
        if (using_prefetch)
        {
            ...
            {
                sprintf (buffer, "%s (regs, %d)", prefetch_word, r + 2);
                count_read++;
                insn_n_cycles += 4;
            }
        }
...
static const char *gen_nextibyte (int flags)
{
    ...
    else
    {
        ...
        if (using_prefetch)
        {
            ...
            {
                sprintf (buffer, "(uae_u8)%s (regs, %d)", prefetch_word, r + 2);
                insn_n_cycles += 4;
                count_read++;
            }
        }
...
static void genamode2x (amodes mode, const char *reg, wordsizes size, const char *name, int getv, int movem, int flags, int fetchmode)
{
    ...
    case Ad8r: // (d8,An,Xn)
        ...
        if (cpu_level > 1)
        {
            ...
            insn_n_cycles += 4;
            printf ("\t%sa = %s (regs, m68k_areg (regs, %s), %d);\n", name, disp020, reg, disp020cnt++);
        }
        else
        {
            if (!(flags & GF_AD8R))
            ...
            {
                printf ("\t%sa = %s (regs, m68k_areg (regs, %s), regs.irc);\n", name, disp000, reg);
            }
            else
            {
                printf ("\t%sa = %s (regs, m68k_areg (regs, %s), %s);\n", name, disp000, reg, gen_nextiword (flags));
            }
            count_read_ea++;
        }
        break;
    case PC8r: // (d8,PC,Xn)
        ...
        else
        {
            ...
            if ((flags & GF_NOREFILL) && using_prefetch)
            {
                printf ("\t%sa = %s (regs, tmppc, regs.irc);\n", name, disp000);
            }
            else
            {
                printf ("\t%sa = %s (regs, tmppc, %s);\n", name, disp000, gen_nextiword (flags));
            }
        }
    ...
    if (getv == 1)
    {
        ...
        else if (using_mmu)
        {
            if (flags & GF_FC)
            {
                ...
                case sz_byte:
                    insn_n_cycles += 4;
                    printf ("\tuae_s8 %s = sfc%s_get_byte (%sa);\n", name, mmu_postfix, name);
                    break;
                case sz_word:
                    insn_n_cycles += 4;
                    printf ("\tuae_s16 %s = sfc%s_get_word (%sa);\n", name, mmu_postfix, name);
                    break;
                case sz_long:
                    insn_n_cycles += 8;
                    printf ("\tuae_s32 %s = sfc%s_get_long (%sa);\n", name, mmu_postfix, name);
                    break;
                default:
                    term ();
                }
            ...
            else
            {
                ...
                case sz_byte:
                    insn_n_cycles += 4;
                    printf ("\tuae_s8 %s = %s (%sa);\n", name, (flags & GF_LRMW) ? srcblrmw : (rmw ? srcbrmw : srcb), name);
                    break;
                case sz_word:
                    insn_n_cycles += 4;
                    printf ("\tuae_s16 %s = %s (%sa);\n", name, (flags & GF_LRMW) ? srcwlrmw : (rmw ? srcwrmw : srcw), name);
                    break;
                case sz_long:
                    insn_n_cycles += 8;
                    printf ("\tuae_s32 %s = %s (%sa);\n", name, (flags & GF_LRMW) ? srcllrmw : (rmw ? srclrmw : srcl), name);
                    break;
                default:
                    term ();
                }
...
static void genamode (instr *curi, amodes mode, const char *reg, wordsizes size, const char *name, int getv, int movem, int flags)
{
    ...
    if (isce020() && curi)
    {
        ...
        //break;
        //}
        genamode2x (mode, reg, size, name, getv, movem, flags, 0 /*curi->fetchmode*/);
    }
    else
    {
...
static void genmovemel_ce (uae_u16 opcode)
{
    ...
    else
    {
        ...
        printf ("\t\tsrca += %d;\n", size);
        printf ("\t\tdmask = movem_next[dmask];\n");
        addcycles000_nonce("\t\t", cpu_level > 1 ? 4 : 4);
        printf ("\t}\n");
        printf ("\twhile (amask) {\n");
    ...
    if (optimized_flags)
    {
        ...
        case flag_logical_noclobber:
            ...
            else
            {
                ...
                {
                case sz_byte:
                    printf ("\toptflag_testb (regs, (uae_s8)(%s));\n", value);
                    break;
                case sz_word:
                    printf ("\toptflag_testw (regs, (uae_s16)(%s));\n", value);
                    break;
                case sz_long:
                    printf ("\toptflag_testl (regs, (uae_s32)(%s));\n", value);
                    break;
                }
        ...
        case flag_add:
            ...
            {
            case sz_byte:
                printf ("\toptflag_addb (regs, %s, (uae_s8)(%s), (uae_s8)(%s));\n", value, src, dst);
                break;
            case sz_word:
                printf ("\toptflag_addw (regs, %s, (uae_s16)(%s), (uae_s16)(%s));\n", value, src, dst);
                break;
            case sz_long:
                printf ("\toptflag_addl (regs, %s, (uae_s32)(%s), (uae_s32)(%s));\n", value, src, dst);
                break;
            }
            return;
        ...
        case flag_sub:
            ...
            {
            case sz_byte:
                printf ("\toptflag_subb (regs, %s, (uae_s8)(%s), (uae_s8)(%s));\n", value, src, dst);
                break;
            case sz_word:
                printf ("\toptflag_subw (regs, %s, (uae_s16)(%s), (uae_s16)(%s));\n", value, src, dst);
                break;
            case sz_long:
                printf ("\toptflag_subl (regs, %s, (uae_s32)(%s), (uae_s32)(%s));\n", value, src, dst);
                break;
            }
            return;
        ...
        case flag_cmp:
            ...
            {
            case sz_byte:
                printf ("\toptflag_cmpb (regs, (uae_s8)(%s), (uae_s8)(%s));\n", src, dst);
                break;
            case sz_word:
                printf ("\toptflag_cmpw (regs, (uae_s16)(%s), (uae_s16)(%s));\n", src, dst);
                break;
            case sz_long:
                printf ("\toptflag_cmpl (regs, (uae_s32)(%s), (uae_s32)(%s));\n", src, dst);
                break;
            }
            return;
...
//	if (i68000)
//		printf("#ifndef CPUEMU_68000_ONLY\n");
    printf ("%s REGPARAM2 CPUFUNC(op_%04x_%d%s)(uae_u32 opcode, struct regstruct &regs)\n{\n", (using_ce || using_ce020) ? "void" : "uae_u32", opcode, postfix, extra);
...
static void generate_cpu (int id, int mode)
{
    ...
    postfix = id;
    if (id == 0 || id == 4 || id == 11 || id == 13 || id == 20 || id == 21 || id == 22 || id == 23 || id == 24 || id == 31 || id == 32 || id == 33 || id == 40)
    {
    ...
    else if (id == 11 || id == 12)     // 11 = 68000 prefetch, 12 = 68000 cycle-exact
    {
        cpu_level = 0;
        using_prefetch = 1;
        using_exception_3 = 1;
