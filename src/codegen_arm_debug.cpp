...
{
  D(bug("%08x: raw_lea_l_brr(%d, %d, %08x)\n", target, d, s, offset));
#if defined(USE_DATA_BUFFER)
...
{
  D(bug("%08x: raw_lea_l_brr_indexed(%d, %d, %d, %d, %08x)\n", target, d, s, index, factor, offset));
	int shft;
	switch(factor) {
	case 1: shft=0; break;
	case 2: shft=1; break;
...
{
  D(bug("%08x: raw_lea_l_rr_indexed(%d, %d, %d, %d)\n", target, d, s, index, factor));
	int shft;
	switch(factor) {
	case 1: shft=0; break;
	case 2: shft=1; break;
...
{
  D(bug("%08x: raw_mov_b_brR(%d, %d, %08x)\n", target, d, s, offset));
#if defined(USE_DATA_BUFFER)
  long offs = data_long_offs(offset);
	LDR_rRI(REG_WORK1, RPC_INDEX, offs); 	// ldr  r2, [pc, #offs]
#else
...
{
  D(bug("%08x: raw_mov_l_brR(%d, %d, %08x)\n", target, d, s, offset));
#if defined(USE_DATA_BUFFER)
  long offs = data_long_offs(offset);
	LDR_rRI(REG_WORK1, RPC_INDEX, offs); 	// ldr     r2, [pc, #offs]
...
LOWFUNC(NONE,WRITE,2,raw_mov_l_mi,(MEMW d, IMM s))
{
  ...
  D(bug("%08x: raw_mov_l_mi(%08x, %08x)\n", target, d, s));
#if defined(USE_DATA_BUFFER)
  data_check_end(8);
  long offs = data_long_offs(d);
...
{
  D(bug("%08x: raw_mov_w_brR(%d, %d, %08x)\n", target, d, s, offset));
#if defined(USE_DATA_BUFFER)
  long offs = data_long_offs(offset);
	LDR_rRI(REG_WORK1, RPC_INDEX, offs); 	// ldr     r2, [pc, #offs]
#else
...
LOWFUNC(NONE,WRITE,2,raw_mov_w_mi,(MEMW d, IMM s))
{
  ...
  D(bug("%08x: raw_mov_w_mi(%08x, %04x)\n", target, d, s));
#if defined(USE_DATA_BUFFER)
  data_check_end(8);
  long offs = data_long_offs(d);
...
{
  D(bug("%08x: raw_mov_w_Ri(%d, %08x, %02x)\n", target, d, i, offset));
...
{
  D(bug("%08x: raw_mov_w_rR(%d, %d, %02x)\n", target, d, s, offset));
	Dif(!isbyte(offset)) abort();
...
{
  D(bug("%08x: raw_mov_l_rm_indexed(%d, %08x, %d, %d)\n", target, d, base, index, factor));
	int shft;
	switch(factor) {
	case 1: shft=0; break;
	case 2: shft=1; break;
...
{
  D(bug("%08x: raw_mov_l_Ri(%d, %08x, %02x)\n", target, d, i, offset8));
...
{
  D(bug("%08x: raw_mov_l_rR(%d, %d, %02x)\n", target, d, s, offset));
	Dif(!isbyte(offset)) abort();
...
{
  D(bug("%08x: raw_sign_extend_16_rr(%d, %d)\n", target, d, s));
#if defined(ARMV6_ASSEMBLY)
...
{
  D(bug("%08x: raw_sign_extend_8_rr(%d, %d)\n", target, d, s));
#if defined(ARMV6_ASSEMBLY)
...
{
  D(bug("%08x: raw_zero_extend_8_rr(%d, %d)\n", target, d, s));
#if defined(ARMV6_ASSEMBLY)
...
{
  D(bug("%08x: raw_zero_extend_16_rr(%d, %d)\n", target, d, s));
#if defined(ARMV6_ASSEMBLY)
...
static inline void raw_flags_init(void) {
  D(bug("%08x: raw_flags_init()\n", target));
}

static __inline__ void raw_flags_set_zero(int s, int tmp)
{
  D(bug("%08x: raw_flags_set_zero(%d, %d)\n", target, s, tmp));
	 raw_mov_l_rr(tmp,s);
	 MRS_CPSR(s);
...
{
  D(bug("%08x: raw_load_flagreg(%08x, %d)\n", target, t, r));
	raw_mov_l_rm(t,(uintptr)live.state[r].mem);
}
...
{
  D(bug("%08x: raw_emit_nop_filler(%d)\n", target, nbytes));
	nbytes >>= 2;
	while(nbytes--) { NOP(); }
}
...
{
  D(bug("%08x: raw_ADD_l_rri(%d, %d, %04x)\n", target, d, s, i));
	ADD_rri(d, s, i);
}
...
{
  D(bug("%08x: raw_SUB_l_rri(%d, %d, %04x)\n", target, d, s, i));
	SUB_rri(d, s, i);
}
...
{
  D(bug("%08x: compemu_raw_add_l_mi(%08x, %08x)\n", target, d, s));
#if defined(USE_DATA_BUFFER)
  data_check_end(8);
  long target = data_long(d);
...
{
  D(bug("%08x: compemu_raw_and_l_ri(%d, %08x)\n", target, d, i));
#if defined(USE_DATA_BUFFER)
  long offs = data_long_offs(i);
	LDR_rRI(REG_WORK1, RPC_INDEX, offs); 	// ldr     r2, [pc, #offs]   ; <value>
...
{
  D(bug("%08x: compemu_raw_bswap_32(%d)\n", target, r));
#if defined(ARMV6_ASSEMBLY)
	REV_rr(r,r);								// rev 	   %[r],%[r]
#else
	EOR_rrrRORi(REG_WORK1, r, r, 16);      		// eor     r2, r6, r6, ror #16
...
{
  D(bug("%08x: compemu_raw_bt_l_ri(%d, %08x)\n", target, r, i));
	int imm = (1 << (i & 0x1f));
...
{
  D(bug("%08x: compemu_raw_cmov_l_rm_indexed(%d, %08x, %d, %d, %d)\n", target, d, base, index, factor, cond));
	int shft;
	switch(factor) {
	case 1: shft=0; break;
	case 2: shft=1; break;
...
{
  D(bug("%08x: compemu_raw_cmp_l_mi(%08x, %08x)\n", target, d, s));
#if defined(USE_DATA_BUFFER)
  data_check_end(8);
  long offs = data_long_offs(d);
	LDR_rRI(REG_WORK1, RPC_INDEX, offs);    	// ldr     r2, [pc, #offs]    ; d
...
{
  D(bug("%08x: compemu_raw_cmp_l_mi8(%08x, %08x)\n", target, d, s));
#if defined(USE_DATA_BUFFER)
  long offs = data_long_offs(d);
	LDR_rRI(REG_WORK1, RPC_INDEX, offs);    // ldr     r2, [pc, #offs]    ; <value>
#else
...
{
  D(bug("%08x: compemu_raw_lea_l_brr(%d, %d, %08x)\n", target, d, s, offset));
#if defined(USE_DATA_BUFFER)
  long offs = data_long_offs(offset);
	LDR_rRI(REG_WORK1, RPC_INDEX, offs); 	// ldr     r2, [pc, #offs]    ; <value>
	ADD_rrr(d, s, REG_WORK1);         	  // add     r7, r6, r2
...
{
  D(bug("%08x: compemu_raw_lea_l_rr_indexed(%d, %d, %d, %d)\n", target, d, s, index, factor));
	int shft;
	switch(factor) {
	case 1: shft=0; break;
	case 2: shft=1; break;
...
{
  D(bug("%08x: compemu_raw_mov_b_mr(%08x, %d)\n", target, d, s));
#if defined(USE_DATA_BUFFER)
  long offs = data_long_offs(d);
	LDR_rRI(REG_WORK1, RPC_INDEX, offs); 	// ldr	r2, [pc, #offs]	; <value>
	STRB_rR(s, REG_WORK1);	           	  // strb	r6, [r2]
...
LOWFUNC(NONE,WRITE,2,compemu_raw_mov_l_mi,(MEMW d, IMM s))
{
  ...
  D(bug("%08x: compemu_raw_mov_l_mi(%08x, %08x)\n", target, d, s));
#if defined(USE_DATA_BUFFER)
  data_check_end(8);
  long offs = data_long_offs(d);
	LDR_rRI(REG_WORK1, RPC_INDEX, offs); 	// ldr    r2, [pc, #offs]    ; d
...
{
  D(bug("%08x: compemu_raw_mov_l_mr(%08x, %d)\n", target, d, s));
#if defined(USE_DATA_BUFFER)
  long offs = data_long_offs(d);
	LDR_rRI(REG_WORK1, RPC_INDEX, offs); 	// ldr     r2, [pc, #offs]    ; <value>
	STR_rR(s, REG_WORK1);             	  // str     r3, [r2]
...
{
  D(bug("%08x: compemu_raw_mov_l_ri(%d, %08x)\n", target, d, s));
#if defined(USE_DATA_BUFFER)
  long offs = data_long_offs(s);
...
{
  D(bug("%08x: compemu_raw_mov_l_rm(%d, %08x)\n", target, d, s));
#if defined(USE_DATA_BUFFER)
  long offs = data_long_offs(s);
	LDR_rRI(REG_WORK1, RPC_INDEX, offs); 	// ldr     r2, [pc, #offs]    ; <value>
	LDR_rR(d, REG_WORK1);              	  // ldr     r7, [r2]
...
{
  D(bug("%08x: compemu_raw_mov_l_rr(%d, %d)\n", target, d, s));
	MOV_rr(d, s); 					// mov     %[d], %[s]
}
...
{
  D(bug("%08x: compemu_raw_mov_w_mr(%08x, %d)\n", target, d, s));
#if defined(USE_DATA_BUFFER)
  long offs = data_long_offs(d);
	LDR_rRI(REG_WORK1, RPC_INDEX, offs);  // ldr     r2, [pc, #offs]    ; <value>
	STRH_rR(s, REG_WORK1);             	  // strh    r3, [r2]
...
{
  D(bug("%08x: compemu_raw_sub_l_mi(%08x, %08x)\n", target, d, s));
#if defined(USE_DATA_BUFFER)
  data_check_end(8);
  long target = data_long(d);
  long offs = get_data_offset(target);
...
{
  D(bug("%08x: compemu_raw_test_l_rr(%d, %d)\n", target, d, s));
	TST_rr(d, s);                          			// tst     r7, r6
}
...
{
  D(bug("%08x: compemu_raw_zero_extend_16_rr(%d, %d)\n", target, d, s));
#if defined(ARMV6_ASSEMBLY)
	UXTH_rr(d, s);									// UXTH %[d], %[s]
#else
	BIC_rri(d, s, 0xff000000); 						// bic	%[d], %[s], #0xff000000
...
{
  D(bug("%08x: compemu_raw_call(%08x)\n", target, t));
#if defined(USE_DATA_BUFFER)
...
{
  D(bug("%08x: compemu_raw_call_r(%d)\n", target, r));
	PUSH(RLR_INDEX);  // push    {lr}
	BLX_r(r);					// blx	   r0
	POP(RLR_INDEX);   // pop     {lr}
}
...
{
  D(bug("%08x: compemu_raw_jcc_l_oponly(%d)\n", target, cc));
	switch (cc) {
	case 9: // LS
		BEQ_i(0);										// beq <dojmp>
		BCC_i(2);										// bcc <jp>
...
{
  D(bug("%08x: compemu_raw_jmp_m_indexed(%08x, %d, %d)\n", target, base, r, m));
	int shft;
	switch(m) {
	case 1: shft=0; break;
	case 2: shft=1; break;
...
{
  D(bug("%08x: compemu_raw_jmp_r(%d)\n", target, r));
	BX_r(r);
}

static inline void compemu_raw_jnz(uae_u32 t)
{
  D(bug("%08x: compemu_raw_jnz(%08x)\n", target, t));
#if defined(USE_DATA_BUFFER)
  long offs = data_long_offs(t);
...
{
  D(bug("%08x: compemu_raw_jz_b_oponly()\n", target));
	BNE_i(2);									// bne jp
...
{
  D(bug("%08x: compemu_raw_branch(%d)\n", target, d));
	B_i((d >> 2) - 1);
}
