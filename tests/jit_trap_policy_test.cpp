 ...
 * released the arithmetic traps, and #2336 released the SR/USP moves, leaving
 * the structural set asserted below.
 *
...
/* Opcodes that MUST demote: structural and CPU-tester shapes. These are cold
 * in ordinary user code, and compiling them alongside interpreted trap opcodes
 * exposes stale lazy-flag state. The CPU tester depends on this set: every
 * test block ends in an ILLEGAL sentinel, so those blocks stay interpreted. */
const Mnemonic kMustDemote[] = {
	{ i_ILLG, "ILLG" }, { i_RTE, "RTE" }, { i_STOP, "STOP" },
	{ i_RESET, "RESET" }, { i_MOVEC2, "MOVEC2" }, { i_MOVE2C, "MOVE2C" },
	{ i_MOVES, "MOVES" }, { i_TRAPcc, "TRAPcc" },
	{ i_FTRAPcc, "FTRAPcc" }, { i_TRAPV, "TRAPV" },
...
/* Opcodes that MUST NOT demote: hot in ordinary user code, and released back
 * to the JIT by #2305 (arithmetic traps, after #2299) and #2336 (SR/USP moves,
 * after #2315). A regression here is the expensive direction - it costs
 * throughput without changing any answer, so only this test catches it.
 *
 * The SR/USP moves are hot because AmigaOS runs everything in supervisor mode:
 * MV2SR/MVSR2/ANDSR/ORSR/EORSR are how exec implements Forbid()/Permit()/
 * Disable()/Enable(). Demoting them cost 28% of SysInfo integer throughput
 * (#2315). MVR2USP/MVUSP2R travel with them. */
const Mnemonic kMustCompile[] = {
	{ i_MV2SR, "MV2SR" }, { i_MVSR2, "MVSR2" },
